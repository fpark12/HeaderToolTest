...
#if 0
static std::vector<unsigned char> combinePath(const char *infile, const char *outfile)
{
...
std::vector<unsigned char> composePreprocessorOutput(const Symbols &symbols) {
...
int runMoc( int _argc, char **_argv )
{
	...
	std::vector<std::vector<unsigned char>> argv;
	argv.resize( _argc - 1 );
	...
	for (int n = 0; n < argv.count(); ++n)
	{
		...
		{
			std::vector<unsigned char> optionsFile = argv.at( n );
			...
			while (!f.atEnd())
			{
				std::vector<unsigned char> line = f.readLine().trimmed();
				if (!line.isEmpty())
					argv.insert( n++, line );
			}
		}
	...
	{
		std::vector<unsigned char> arg( argv[n] );
		if (arg[0] != '-')
		{
			if (filename.isEmpty())
			{
		...
		std::vector<unsigned char> opt = arg.mid( 1 );
