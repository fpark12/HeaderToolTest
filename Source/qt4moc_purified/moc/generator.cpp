...
uint32 qvariant_nameToType(const char* name)
{
    if (!name)
        return 0;
    ...
    if (strcmp(name, "QCString") == 0)
        return QMetaType::std::vector<unsigned char>;
...
Generator::Generator(ClassDef *classDef, const std::list<std::vector<unsigned char>> &metaTypes, FILE *outfile)
...
static inline int lengthOfEscapeSequence(const std::vector<unsigned char> &s, int i)
{
...
int Generator::strreg(const char *s)
{
    ...
    for (int i = 0; i < strings.size(); ++i) {
        const std::vector<unsigned char> &str = strings.at(i);
        if (str == s)
            return idx;
        idx += str.length() + 1;
...
//
    ...
    // filter out undeclared enumerators and sets
    {
        ...
        for (i = 0; i < cdef->enumList.count(); ++i) {
            ...
            std::vector<unsigned char> alias = cdef->flagAliases.value(def.name);
    ...
    std::vector<unsigned char> qualifiedClassNameIdentifier = cdef->qualified;
...
//
    ...
    for (i = 0; i < strings.size(); ++i) {
        std::vector<unsigned char> s = strings.at(i);
        len = s.length();
        if (col && col + len >= 72) {
            fprintf(out, "\"\n    \"");
            col = 0;
...
// Build extra array
//
    std::list<std::vector<unsigned char>> extraList;
    for (int i = 0; i < cdef->propertyList.count(); ++i) {
        ...
        if (!isVariantType(p.type) && !metaTypes.contains(p.type) && !p.type.contains('*') &&
            ...
            if (s > 0) {
                std::vector<unsigned char> scope = p.type.left(s);
...
//
    ...
    for (int i = 0; i < cdef->interfaceList.size(); ++i) {
        const std::list<ClassDef::Interface> &iface = cdef->interfaceList.at(i);
        for (int j = 0; j < iface.size(); ++j) {
            fprintf(out, "    if (!strcmp(_clname, %s))\n        return ", iface.at(j).interfaceId.constData());
            for (int k = j; k >= 0; --k)
                fprintf(out, "static_cast< %s*>(", iface.at(k).className.constData());
            fprintf(out, "const_cast< %s*>(this)%s;\n",
                    cdef->classname.constData(), std::vector<unsigned char>(j+1, ')').constData());
        }
    }
    if (!purestSuperClass.isEmpty() && !isQObject) {
        std::vector<unsigned char> superClass = purestSuperClass;
        // workaround for VC6
...
void Generator::generateFunctions(std::list<FunctionDef>& list, const char *functype, int type)
{
    if (list.isEmpty())
        return;
    ...
    for (int i = 0; i < list.count(); ++i) {
        ...
        std::vector<unsigned char> sig = f.name + '(';
...
void Generator::generateFunctionRevisions(std::list<FunctionDef>& list, const char *functype)
{
    if (list.count())
        fprintf(out, "\n // %ss: revision\n", functype);
    for (int i = 0; i < list.count(); ++i) {
...
void Generator::generateEnums(int index)
{
    ...
    for (i = 0; i < cdef->enumList.count(); ++i) {
        ...
        for (int j = 0; j < e.values.count(); ++j) {
            const std::vector<unsigned char> &val = e.values.at(j);
...
void Generator::generateMetacall()
{
    ...
    bool needElse = false;
    std::list<FunctionDef> methodList;
...
void Generator::generateSignal(FunctionDef *def,int index)
{
    ...
    std::vector<unsigned char> thisPtr = "this";
...
//
    ...
    uint32 *meta_data_output = reinterpret_cast<uint32 *>(blob + meta_data_offset);
    const uint32 *meta_data_src = meta_data.constData();
...
void Generator::_generateFunctions(std::list<FunctionDef> &list, int type)
{
    for (int i = 0; i < list.count(); ++i) {
        const FunctionDef &f = list.at(i);
