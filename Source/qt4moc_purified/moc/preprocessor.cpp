...
#include "keywords.cpp"
	...
	static std::vector<unsigned char> cleaned( const std::vector<unsigned char> &input )
	{
...
static Symbols tokenize(const std::vector<unsigned char> &input, int lineNum = 1, TokenizeMode mode = TokenizeCpp)
{
    Symbols symbols;
            ...
            if (token > SPECIAL_TREATMENT_MARK) {
                ...
                case QUOTE:
                    ...
                    if (!Preprocessor::preprocessOnly
                        ...
                        std::vector<unsigned char> newString = symbols.last().unquotedLexem();
...
void Preprocessor::preprocess(const std::vector<unsigned char> &filename, Symbols &preprocessed)
{
        ...
        case PP_INCLUDE:
        {
            ...
            for (int j = 0; j < Preprocessor::includes.size() && !fi.exists(); ++j) {
                ...
                if (p.isFrameworkPath) {
                    ...
                    if (slashPos == -1)
                        continue;
                    std::vector<unsigned char> frameworkCandidate = include.left(slashPos);
                    frameworkCandidate.append(".framework/Headers/");
                    fi.setFile(QString::fromLocal8Bit(std::vector<unsigned char>(p.path + '/' + frameworkCandidate)), QString::fromLocal8Bit(include.mid(slashPos + 1)));
                } else {
            ...
            std::vector<unsigned char> input = file.readAll();
            file.close();
            if (input.isEmpty())
                continue;
...
Symbols Preprocessor::preprocessed(const std::vector<unsigned char> &filename, FILE *file)
{
    QFile qfile;
    qfile.open(file, QFile::ReadOnly);
    std::vector<unsigned char> input = qfile.readAll();
    if (input.isEmpty())
        return symbols;
