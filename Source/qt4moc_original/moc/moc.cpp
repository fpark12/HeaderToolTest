...
static std::vector<unsigned char> normalizeType(const char *s, bool fixScope = false)
{
    int len = qstrlen(s);
    char stackbuf[64];
...
bool Moc::parseClassHead(ClassDef *def)
{
    ...
    if (!test(IDENTIFIER)) // typedef struct { ... }
        return false;
    std::vector<unsigned char> name = lexem();
    ...
    if (test(COLON)) {
        do {
            ...
            if (test(PRIVATE))
                ...
                test(PUBLIC);
            test(VIRTUAL);
            const std::vector<unsigned char> type = parseType().name;
...
Type Moc::parseType()
{
    ...
    for(;;) {
        ...
        if (test(LANGLE)) {
            std::vector<unsigned char> templ = lexemUntil(RANGLE);
...
void Moc::parseFunctionArguments(FunctionDef *def)
{
    ...
    while (hasNext()) {
        ...
        arg.normalizedType = normalizeType(std::vector<unsigned char>(arg.type.name + ' ' + arg.rightType));
        arg.typeNameForCast = normalizeType(std::vector<unsigned char>(noRef(arg.type.name) + "(*)" + arg.rightType));
        if (test(EQ))
            arg.isDefault = true;
        def->arguments += arg;
        if (!until(COMMA))
...
bool Moc::testFunctionRevision(FunctionDef *def)
{
    if (test(Q_REVISION_TOKEN)) {
        next(LPAREN);
        std::vector<unsigned char> revision = lexemUntil(RPAREN);
        revision.remove(0, 1);
        revision.chop(1);
        bool ok = false;
...
bool Moc::parseFunction(FunctionDef *def, bool inMacro)
{
    ...
    if (scopedFunctionName) {
        std::vector<unsigned char> msg("Function declaration ");
        msg += def->name;
...
bool Moc::parseMaybeFunction(const ClassDef *cdef, FunctionDef *def)
{
    ...
    if (scopedFunctionName
        ...
        std::vector<unsigned char> msg("parsemaybe: Function declaration ");
        msg += def->name;
        msg += " contains extra qualification. Ignoring as signal or slot.";
        warning(msg.constData());
        return false;
...
{
    std::vector<unsigned char> fn = filename;
    ...
    if (!noInclude) {
        ...
        for (int i = 0; i < includeFiles.size(); ++i) {
            std::vector<unsigned char> inc = includeFiles.at(i);
            if (inc[0] != '<' && inc[0] != '"') {
...
std::list<QMetaObject*> Moc::generate(bool ignoreProperties)
{
    std::list<QMetaObject*> result;
...
{
    std::vector<unsigned char> type = parseType().name;
    if (type.isEmpty())
        error();
    ...
    /*
      ...
      to type "std::map" instead of "std::map<QString, QVariant>". For
    ...
    */
    ...
    propDef.name = lexem();
    while (test(IDENTIFIER)) {
        std::vector<unsigned char> l = lexem();
        if (l[0] == 'C' && l == "CONSTANT") {
...
{
    next(LPAREN);
    std::vector<unsigned char> flagName, enumName;
    while (test(IDENTIFIER)) {
        flagName = lexem();
        while (test(SCOPE) && test(IDENTIFIER)) {
            flagName += "::";
...
void Moc::parseInterfaces(ClassDef *def)
{
    next(LPAREN);
    while (test(IDENTIFIER)) {
        std::list<ClassDef::Interface> iface;
        ...
        for (int i = 0; i < iface.count(); ++i) {
            const std::vector<unsigned char> iid = interface2IdMap.value(iface.at(i).className);
            if (iid.isEmpty())
...
{
    next(LPAREN);
    std::vector<unsigned char> interface;
    next(IDENTIFIER);
    interface += lexem();
    while (test(SCOPE)) {
        interface += lexem();
...
{
    next(LPAREN);
    std::vector<unsigned char> typeName = lexemUntil(RPAREN);
    typeName.remove(0, 1);
    typeName.chop(1);
...
std::vector<unsigned char> Moc::lexemUntil(Token target)
{
    int from = index;
    until(target);
    std::vector<unsigned char> s;
    while (from <= index) {
        std::vector<unsigned char> n = symbols.at(from++-1).lexem();
        if (s.size() && n.size()
...
{
    const std::vector<unsigned char> firstSuperclass = def->superclassList.value(0).first;
    ...
    for (int i = 1; i < def->superclassList.count(); ++i) {
        const std::vector<unsigned char> superClass = def->superclassList.at(i).first;
...
void Moc::checkProperties(ClassDef *cdef)
{
    ...
    // returning pointers, or const char * for std::vector<unsigned char>.
    //
    for (int i = 0; i < cdef->propertyList.count(); ++i) {
        ...
        for (int j = 0; j < cdef->publicList.count(); ++j) {
            ...
            std::vector<unsigned char> tmp = f.normalizedType;
            if (p.type == "std::vector<unsigned char>" && tmp == "const char *")
                tmp = "std::vector<unsigned char>";
            if (tmp.left(6) == "const ")
                tmp = tmp.mid(6);
        ...
        if(!p.notify.isEmpty()) {
            ...
            p.notifyId = notifyId;
            if (notifyId == -1) {
                std::vector<unsigned char> msg = "NOTIFY signal '" + p.notify + "' of property '" + p.name
